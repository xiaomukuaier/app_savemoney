...
async def test_stt_with_audio_file():
    ...
    print(f"OpenAI Base URL: {base_url if base_url else '默认'}")
    ...
    try:
        ...
        if stt_service.client is None:
            print("\n⚠️  警告: 使用模拟模式 (OpenAI API密钥未配置或配置错误)")
            print("请检查 OPENAI_API_KEY 环境变量配置")
        else:
            print("\n✅ 成功调用OpenAI Whisper API")

        # 验证结果
        expected_phrases = ["25.3", "毛钱", "午饭", "中午", "25", "3"]
        found_phrases = []
